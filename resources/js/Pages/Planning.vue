        ...
        <div v-if="allPlannings && members && member">
                <div class="min-h-screen">
                    ...
                    <div class="p-4 gap-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-7 select-none">
                        <div v-for="(planning, index) in allPlannings" :key="planning.date" :class="[planning.type === 'CP' ? 'bg-blue-200' : '', planning.type === 'Iti' ? 'bg-red-200' : '',  planning.type !== 'Iti' &&  planning.type !== 'CP' && !planning.today ? 'bg-green-100' : '', planning.today ? 'shadow-blue-400/80 shadow-2xl bg-gray-50' : '']" class="w-full cursor-pointer rounded-md shadow-md shadow-dark hover:shadow-blue-400/80 hover:shadow-2xl hover:bg-gray-50">
                            <div class="p-4">
                                <div class="flex justify-between">
                                    <div>
                                        <p class="font-semibold text-sm py-2">{{ planning.date }}</p>
                                    </div>
                                    <div @click="viewDate(planning.date_id, index)">
