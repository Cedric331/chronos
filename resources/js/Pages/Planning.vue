                ...
                <div class="min-h-screen bg-gray-700">
                    ...
                    <div class="p-2 gap-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-8 select-none">
                        <div v-for="(planning, index) in allPlannings" :key="index" :style="colorPlanning(planning)" :class="[planning.today && !this.isSelected(planning) ? ' shadow-blue-400/80 shadow-2xl bg-white border-solid border-4 border-blue-500' : '', planning.events !== null ? 'striped' : '', planning.today && this.isSelected(planning) ? ' shadow-blue-400/80 shadow-2xl bg-white': '', this.isSelected(planning) ? ' border-4 border-red-500' : '', this.$page.props.auth.user.coordinateur ? ' cursor-pointer' : '']" class="hover:scale-105 relative w-full rounded-md shadow-md shadow-dark hover:shadow-red-400/80 hover:shadow-2xl hover:bg-white">
                            ...
                            <div v-else>
                                <div v-if="!planning.flipped" class="front">
                                    ...
                                    <div v-if="planning.events !== null" @click="flip(index)" class="flex justify-end p-1 absolute bottom-0 right-0">
                                ...
                                <div v-else class="back">
                                    <div class="absolute bottom-0 right-0">
                                        <div @click="flip(index)" class="flex justify-end p-1">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                                <g stroke-width="1">
                                    ...
                                    <div class="p-2 mb-3" v-if="planning.events !== null">
                                        <div>
                                            <p class="font-bold text-sm py-1 underline" :style="texte">Évènements</p>
                                        </div>
                                        <div v-for="event in planning.events">
                                            <p class="flex justify-between font-semibold p-1" :style="texte">
                                                <span class="font-bold">{{ event.name }} : {{ event.heure_debut }} - {{ event.heure_fin }}</span>
...
export default {
    ...
    methods: {
        flip (index) {
            this.plannings[index].flipped = !this.plannings[index].flipped
        },
        handleResize () {
